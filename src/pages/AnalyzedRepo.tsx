...
const AnalyzedRepo: React.FC = () => {
    ...
    if (!job) return (
        <div className="h-screen bg-bram-bg flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-white font-bold text-sm uppercase tracking-widest animate-pulse">Initializing Environment...</p>
        </div>
    );
    ...
    return (
        <div className="h-screen bg-bram-bg flex flex-col overflow-hidden p-10 font-sans antialiased text-left relative">
            ...
            <div className="w-full max-w-7xl mx-auto mb-10 bg-white p-8 rounded-[2.5rem] border-2 border-bram-border shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-8">
                    ...
                    <div>
                        ...
                        <p className="text-bram-text-muted font-bold text-[12px] uppercase tracking-wider mt-1">
                            {job.targetCloud} • {job.computeType}
                        </p>
                    </div>
                </div>
                <div className={`px-10 py-3 rounded-full font-bold text-xs border-2 uppercase tracking-widest
            ...
            {/* TERMINAL VIEWS - Updated font handling */}
            <div className="w-full max-w-7xl mx-auto flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10 min-h-0">
                <div className="bg-[#0f172a] rounded-[2rem] border-2 border-white/10 shadow-2xl flex flex-col overflow-hidden">
                    <div className="bg-slate-800/50 px-8 py-4 border-b border-white/5 flex items-center gap-4">
                        ...
                        <span className="font-bold text-[10px] uppercase text-slate-400 tracking-widest">Analysis_Logs</span>
                    </div>
                ...
                <div className="bg-[#0f172a] rounded-[2rem] border-2 border-white/10 shadow-2xl flex flex-col overflow-hidden">
                    <div className="bg-slate-800/50 px-8 py-4 border-b border-white/5 flex items-center gap-4">
                        ...
                        <span className="font-bold text-[10px] uppercase text-slate-400 tracking-widest">Blueprint.json</span>
                    </div>
                ...
                <div className="flex-1 flex gap-12">
                    {serviceStatuses.map((svc) => (
                        <div key={svc.name} className="flex items-center gap-3">
                            ...
                            <span className="text-[11px] font-bold uppercase text-slate-500 tracking-wide">{svc.name}</span>
                        </div>
                    ))}
                </div>
                <div className="flex gap-4">
                    <button onClick={handleDownloadMaster} disabled={!isReadyForExecution || isDownloading} className="px-8 py-4 rounded-3xl font-bold text-xs uppercase bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 transition-all flex items-center gap-3 shadow-lg">
                        {isDownloading ? <Loader2 className="animate-spin" size={20} /> : <Download size={20} />} Package
                    </button>
                    <button onClick={handleOpenReview} disabled={!isReadyForExecution || isFetchingDiff} className="px-10 py-4 rounded-3xl font-bold text-xs uppercase bg-bram-primary text-white hover:bg-blue-700 disabled:opacity-50 transition-all flex items-center gap-3 shadow-lg">
                        {isFetchingDiff ? <Loader2 className="animate-spin" size={20} /> : <Play size={20} />} Review & Deploy
                    </button>
                </div>
            </div>

            {/* MODALS (Review & Confirm) - Clearer Typography */}
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-4 mt-1">
                                        <span className="text-[9px] font-bold text-red-400 uppercase tracking-widest bg-red-400/10 px-3 py-1 rounded-full border border-red-400/10">AI Draft</span>
                                        <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest bg-emerald-400/10 px-3 py-1 rounded-full border border-emerald-400/10">Validated</span>
                                    </div>
                                </div>
                            </div>
                            <button onClick={() => setIsReviewOpen(false)} className="text-slate-500 hover:text-white p-3 bg-white/5 rounded-full"><X size={24} /></button>
                        </div>
                        <div className="flex-1 flex min-h-0">
                                ...
                                {Object.entries(categories).map(([catName, files]) => (
                                    <div key={catName} className="mb-8">
                                        <div className="flex items-center gap-3 mb-3 px-2 opacity-40">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-widest text-white">{catName}</span>
                                        </div>
                                        {files.map(file => (
                                            <button key={file.filename} onClick={() => setSelectedFile(file)} className={`w-full text-left px-4 py-2.5 rounded-xl text-[11px] uppercase font-bold mb-1 tracking-tight ${selectedFile.filename === file.filename ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-white/5'}`}>
                            ...
                            <div className="flex-1 flex flex-col p-6">
                                <div className="mb-4 flex justify-between items-end">
                                    <div><span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest block">{activeCategory}</span><h3 className="text-lg font-bold text-white uppercase">{selectedFile.filename.split(': ')[1]}</h3></div>
                                    <div className="text-[10px] font-bold text-slate-500 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5 uppercase">Language: {selectedFile.language}</div>
                                </div>
                                <div className="flex-1 border border-white/5 rounded-[2rem] overflow-hidden bg-black p-4 shadow-2xl">
                                    <DiffEditor original={selectedFile.draftContent} modified={selectedFile.validatedContent} language={selectedFile.language} theme="vs-dark" options={{ minimap: { enabled: false }, fontSize: 13, wordWrap: "on" }} />
                                </div>
                            </div>
                        </div>
                        <div className="px-10 py-6 border-t border-white/5 bg-white/5 flex justify-end gap-4">
                            <button onClick={() => setIsReviewOpen(false)} className="px-8 py-3 rounded-2xl font-bold text-[11px] uppercase tracking-widest text-slate-400 hover:bg-white/10 transition-all">Close</button>
                            <button onClick={() => setIsConfirmModalOpen(true)} className="px-10 py-4 rounded-2xl font-bold text-[11px] uppercase tracking-widest bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-lg flex items-center gap-3">
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-6">
                    <div className="bg-white border-2 border-slate-200 w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <p className="text-slate-500 text-sm font-medium mb-10 leading-relaxed">Do you want to <span className="font-bold text-slate-900 uppercase underline decoration-emerald-400">commit</span> these validated files to your repository?</p>
                            <div className="w-full flex flex-col gap-3">
                                <button onClick={handleYesCommit} className="w-full py-5 rounded-2xl bg-bram-primary text-white font-bold text-xs uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center justify-center gap-3">
                                    <GitCommit size={18} /> Yes, Commit to my Repo
                                </button>
                                <button onClick={handleNoDownload} className="w-full py-5 rounded-2xl bg-slate-100 text-slate-600 font-bold text-xs uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center justify-center gap-3">
                                    <Download size={18} /> No, Download Locally & Exit
                                </button>
                                <button onClick={() => setIsConfirmModalOpen(false)} className="mt-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-all">Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
