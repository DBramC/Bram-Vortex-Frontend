...
import {
    ...
    Layers, FileCode, HelpCircle, GitCommit
} from 'lucide-react';
...
const AnalyzedRepo: React.FC = () => {
    const { jobId } = useParams<{ jobId: string }>(); // Το jobId είναι UUID (String)
    ...
    useEffect(() => {
        const fetchJobStatus = async () => {
            ...
            try {
                ...
                // Το polling σταματάει όταν το job ολοκληρωθεί ή αποτύχει
    ...
    const handleInitiateDeployment = () => setIsConfirmModalOpen(true);
    ...
    const handleYesCommit = async () => {
        ...
        setIsDeploying(true);
        try {
            // Κλήση στον Orchestrator (Repo Analyzer)
    ...
    return (
        <div className="h-screen bg-bram-bg flex flex-col overflow-hidden p-10 font-sans antialiased text-left relative">
            ...
            <div className="w-full max-w-7xl mx-auto bg-white rounded-[2.5rem] border-2 border-bram-border p-8 shadow-xl flex flex-col md:flex-row items-center gap-10">
                <div className="flex-1 flex gap-12 italic">
                ...
                <div className="flex gap-4">
                    <button onClick={handleDownloadMaster} disabled={!isReadyForExecution || isDownloading} className="px-8 py-4 rounded-3xl font-black text-xs uppercase bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 transition-all shadow-lg">
                        {isDownloading ? <Loader2 className="animate-spin" size={20} /> : <Download size={20} />} Package
                    </button>
                    <button onClick={handleOpenReview} disabled={!isReadyForExecution || isFetchingDiff} className="px-10 py-4 rounded-3xl font-black text-xs uppercase bg-bram-primary text-white hover:bg-blue-700 disabled:opacity-50 transition-all shadow-lg shadow-blue-200">
            ...
            {isReviewOpen && selectedFile && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 backdrop-blur-xl p-4 lg:p-6 overflow-hidden">
                    <div className="bg-[#0f172a] border border-white/10 w-full max-w-[98vw] h-full max-h-[96vh] rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                        <div className="px-10 py-6 border-b border-white/5 bg-white/5 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsReviewOpen(false)} className="text-slate-500 hover:text-white transition-all p-3 bg-white/5 hover:bg-white/10 rounded-full"><X size={24} /></button>
                        </div>
                        <div className="flex-1 flex min-h-0">
                            <div className="w-64 border-r border-white/5 bg-black/20 flex flex-col p-6 overflow-y-auto scrollbar-hide">
                                {Object.entries(categories).map(([catName, files]) => (
                                    <div key={catName} className="mb-8">
                                        <div className="flex items-center gap-3 mb-3 px-2 opacity-40 italic">
                                            <Layers size={12} className="text-blue-400" />
                                            <span className="text-[9px] font-black uppercase tracking-widest text-white">{catName}</span>
                                        </div>
                                        <div className="space-y-1">
                                            {files.map((file) => (
                                                <button key={file.filename} onClick={() => { setSelectedFile(file); setActiveCategory(catName); }} className={`w-full text-left px-4 py-2.5 rounded-xl transition-all flex items-center justify-between group ${selectedFile.filename === file.filename ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:bg-white/5'}`}>
                                                    <div className="flex items-center gap-2.5 min-w-0"><FileCode size={12} /><span className="text-[10px] font-bold truncate uppercase tracking-tight">{file.filename.split(': ')[1]}</span></div>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <div className="flex-1 flex flex-col p-6 bg-[#0f172a]">
                                <div className="mb-4 flex justify-between items-end italic">
                                    <div><span className="text-[9px] font-black text-blue-500 uppercase tracking-widest block">{activeCategory}</span><h3 className="text-lg font-black text-white uppercase tracking-tight">{selectedFile.filename.split(': ')[1]}</h3></div>
                                    <div className="text-[9px] font-bold text-slate-500 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5 uppercase">Language: {selectedFile.language}</div>
                                </div>
                                <div className="flex-1 border border-white/5 rounded-[2rem] overflow-hidden bg-[#050505] shadow-2xl p-4">
                                    <DiffEditor original={selectedFile.draftContent} modified={selectedFile.validatedContent} language={selectedFile.language} theme="vs-dark" options={{ readOnly: true, renderSideBySide: true, minimap: { enabled: false }, fontSize: 13, wordWrap: "on" }} />
                                </div>
                            </div>
                        </div>
                        <div className="px-10 py-6 border-t border-white/5 bg-white/5 flex justify-end gap-4 italic">
                            <button onClick={() => setIsReviewOpen(false)} className="px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest text-slate-400 hover:bg-white/10 transition-all">Close</button>
                            <button onClick={handleInitiateDeployment} disabled={isDeploying || job.status === 'EXECUTING'} className="px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-lg transition-all flex items-center gap-3">
                                {isDeploying ? <Loader2 className="animate-spin" size={18} /> : <Play size={18} />} Approve & Deploy
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-6">
                    <div className="bg-white border-2 border-slate-200 w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex flex-col items-center text-center italic">
                            ...
                            <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase mb-2">Final Confirmation</h3>
                            <p className="text-slate-500 text-sm font-medium mb-10 leading-relaxed">Do you want to <span className="font-bold text-slate-900 uppercase">commit</span> אhese validated infrastructure files directly to your GitHub repository?</p>
                            <div className="w-full flex flex-col gap-3">
                                <button onClick={handleYesCommit} className="w-full py-5 rounded-2xl bg-bram-primary text-white font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center justify-center gap-3 shadow-lg shadow-blue-200">
                                    <GitCommit size={18} /> Yes, Commit to my Repo
                                </button>
                                <button onClick={handleNoDownload} className="w-full py-5 rounded-2xl bg-slate-100 text-slate-600 font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center justify-center gap-3">
                                    <Download size={18} /> No, Download Locally & Exit
                                </button>
                                <button onClick={() => setIsConfirmModalOpen(false)} className="mt-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600">Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
